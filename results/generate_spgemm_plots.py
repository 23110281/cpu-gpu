...
def plot_metric_vs_density(df, out_dir, metric='agg_tflops', ylabel='TFLOP/s', filename_prefix='fig1_tflops'):
    ...
    for mode in modes:
        ...
        plt.savefig(os.path.join(out_dir, f'{filename_prefix}_vs_density_{mode}.png'), dpi=300, bbox_inches='tight')
        plt.close()

def plot_metric_vs_size(df, out_dir, metric='agg_gbps', ylabel='GB/s', filename_prefix='fig2_gbps'):
    ...
    for mode in modes:
        ...
        plt.tight_layout(rect=[0, 0, 1, 0.95])
        plt.savefig(os.path.join(out_dir, f'{filename_prefix}_vs_size_{mode}.png'), dpi=300, bbox_inches='tight')
        plt.close()
...
def plot_power_util(df, out_dir):
    ...
    for mode in modes:
        ...
        plt.tight_layout(rect=[0, 0, 1, 0.95])
        plt.savefig(os.path.join(out_dir, f'fig4_power_util_{mode}.png'), dpi=300, bbox_inches='tight')
        plt.close()
...
def main():
    ...
    set_style()
    plot_metric_vs_density(df, out_dir, metric='agg_tflops', ylabel='TFLOP/s', filename_prefix='fig1_tflops')
    plot_metric_vs_size(df, out_dir, metric='agg_gbps', ylabel='GB/s', filename_prefix='fig2_gbps')
